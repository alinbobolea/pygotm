...
def figure_staggered_grid(nlev: int = 6) -> None:
    ...
    # Vertical axis line
    ax.annotate(
        "", xy=(x_axis, 1.09), xytext=(x_axis, -0.03),
    ...
    for i in range(nlev + 1):
        ...
        if i == 0:
            ...
            label = rf"$i={i}$"
        ax.text(x_axis - 0.09, yi, label, ha="right", va="center",
                fontsize=9, bbox=_lbbox)

    # Layer centres: open squares (mean-flow quantities)
    for i in range(1, nlev + 1):
        yc = (y(i - 1) + y(i)) / 2.0
        ax.plot(x_axis, yc, "ws", ms=10, markeredgecolor="k",
    ...
    x_arrow = x_axis + 0.22
    ax.annotate(
        "", xy=(x_arrow, y(mid)), xytext=(x_arrow, y(mid - 1)),
        arrowprops=dict(arrowstyle="<->", color="dimgray", lw=1.0),
    )
    ax.text(x_arrow + 0.04, (y(mid) + y(mid - 1)) / 2,
            r"$h_i$", va="center", fontsize=10, color="dimgray")

    # Surface label — above the dashed line, left-aligned to avoid right clipping
    ax.text(x_axis, y(nlev) + 0.07, r"$z = \zeta(t)$  (surface)",
            ha="center", va="bottom", fontsize=9.5, color="steelblue")
    # Bottom label — well below dashed line so legend doesn't overlap it
    ax.text(x_axis, y(0) - 0.10, r"$z = -H$  (bottom)",
            ha="center", va="top", fontsize=9.5, color="saddlebrown")
    ...
    ax.axhline(y(nlev), xmin=0.40, xmax=0.72, color="steelblue",
               lw=0.8, ls="--", zorder=1)
    ax.axhline(y(0), xmin=0.40, xmax=0.72, color="saddlebrown",
               lw=0.8, ls="--", zorder=1)
    ...
    iface_handle = plt.Line2D(
        ...
        markersize=8, label=r"interfaces $i=0,\dots,N$: $k,\,\varepsilon,\,\nu_t,\,\kappa_t$",
    )
    ...
    ax.legend(handles=[iface_handle, centre_handle],
              loc="center right", bbox_to_anchor=(1.0, 0.62),
              fontsize=8.5, framealpha=0.95)

    ax.set_xlim(-0.20, 1.05)
    ax.set_ylim(-0.32, 1.45)
    ax.set_title("Staggered Vertical Grid", fontsize=12,
...
def figure_crank_nicolson(sigma: float = 0.6) -> None:
    ...
    ax.annotate(
        "", xy=(1.0, y_axis), xytext=(0.0, y_axis),
        arrowprops=dict(arrowstyle="-|>", color="black", lw=1.3),
        annotation_clip=False,
    )
    ax.text(1.02, y_axis, r"$t$", va="center", fontsize=12)

    for x, lt, lb, col in zip(ts, labels_top, labels_bot, colors):
        # Tick
        ...
        # Label above
        ax.text(x, y_axis + 0.15, lt, ha="center", va="bottom",
                fontsize=8.5, color="black")
        # Label below
        ax.text(x, y_axis - 0.15, lb, ha="center", va="top",
                fontsize=8, color=col)

    # Δt brace
    ax.annotate(
        "", xy=(ts[3], y_axis + 0.32), xytext=(ts[0], y_axis + 0.32),
        arrowprops=dict(arrowstyle="<->", color="black", lw=1.0),
    )
    ax.text((ts[0] + ts[3]) / 2, y_axis + 0.36, r"$\Delta t$",
            ha="center", va="bottom", fontsize=10)

    # sigma note
    ax.text(0.5, 0.04,
            rf"Crank–Nicolson parameter: $\sigma = {sigma}$ (pyGOTM uses $\sigma = 1$, fully implicit)",
            ha="center", va="bottom", fontsize=8, style="italic",
            transform=ax.transAxes, color="dimgray")

    ax.set_xlim(-0.03, 1.05)
    ax.set_ylim(0.0, 1.08)
    ax.set_title("Crank–Nicolson Time Stepping", fontsize=11,
