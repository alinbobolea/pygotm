...
def stage_validation_test_cases_summary(
    *,
    ...
    helper writes an include file from ``validation/report/report.json`` during
    Sphinx configuration import, avoiding stale checked-in timestamps and
    counts.
    ...
    lines = [
        ...
        "``validation/report/report.html`` snapshot,",
...
def stage_validation_html(*, src: Path, staged_root: Path) -> list[Path]:
    ...
    output. The root ``validation/`` directory contains NetCDF outputs,
    reference data, and per-case run directories, so docs build from the curated
    ``validation/report/`` snapshot and stage only HTML files into the desired
    build-output layout::
    ...
    Args:
        src: The ``validation/report/`` directory at the repository root.
