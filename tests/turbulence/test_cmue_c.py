"""Tests for pygotm.turbulence.cmue_c."""
...
def _reference_cmue_c(
    ...
    *,
    ...
    for i in range(1, nlev):
        ...
        as_max = (
            d0 * n0
            + (d0 * n1 + d1 * n0) * out_an[i]
            + (d1 * n1 + d4 * n0) * out_an[i] ** 2
            + d4 * n1 * out_an[i] ** 3
        ) / (d2 * n0 + (d2 * n1 + d3 * n0) * out_an[i] + d3 * n1 * out_an[i] ** 2)
        out_as[i] = min(out_as[i], _AS_LIMIT_FACT * as_max)
...
def _run_step_cmue_c(
    ...
    *,
    ...
    ws = CmueCWorkspace(nlev, n_cols=n_cols)
...
def test_import() -> None:
    assert callable(step_cmue_c)


def test_workspace_instantiates() -> None:
    workspace = CmueCWorkspace(_NLEV, n_cols=2)
    assert workspace.cmue2.shape == (2, _NLEV + 1)


def test_formula_matches_reference_with_clipping() -> None:
    nlev = _NLEV
    state = _make_state(nlev)

    as_ = np.full(nlev + 1, 100.0, dtype=np.float64)
    an = np.full(nlev + 1, -10.0, dtype=np.float64)
    ...
    expected_as, expected_an, expected_1, expected_2 = _reference_cmue_c(
        state,
        as_=as_,
        an=an,
        cmue1=cmue1,
...
def test_multicolumn_parity_for_identical_columns() -> None:
    nlev = _NLEV
    as_ = np.full(nlev + 1, 60.0, dtype=np.float64)
    an = np.full(nlev + 1, -8.0, dtype=np.float64)

    single_state = _make_state(nlev)
    single = _run_step_cmue_c(single_state, nlev, as_=as_, an=an)
    multi_state = _make_state(nlev)
    multi = _run_step_cmue_c(multi_state, nlev, as_=as_, an=an, n_cols=2)
...
def test_no_nan_or_inf_for_valid_inputs() -> None:
    ...
    _run_step_cmue_c(
        state,
        nlev,
        as_=np.full(nlev + 1, 80.0, dtype=np.float64),
        an=np.full(nlev + 1, -12.0, dtype=np.float64),
    )
