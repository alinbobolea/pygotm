...
def test_validate_case_skip_run_missing_nc_returns_error(tmp_path: Path) -> None:
    with patch("pygotm.validation.runner._ref_path", return_value=tmp_path / "ref.nc"), \
         patch("pygotm.validation.runner._yaml_path", return_value=tmp_path / "gotm.yaml"):
...
def test_validate_case_skip_run_existing_nc(tmp_path: Path) -> None:
    ...
    with patch("pygotm.validation.runner._ref_path", return_value=ref_nc), \
         patch("pygotm.validation.runner._yaml_path", return_value=tmp_path / "gotm.yaml"):
...
def test_validate_case_threads_on_step_to_run_case(tmp_path: Path) -> None:
    ...
    with patch("pygotm.validation.runner.run_case", side_effect=fake_run_case), \
...
def test_validate_case_run_exception_returns_error(tmp_path: Path) -> None:
    ...
    with patch("pygotm.validation.runner.run_case", side_effect=RuntimeError("boom")), \
         patch("pygotm.validation.runner._ref_path", return_value=ref_nc):
